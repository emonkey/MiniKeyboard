...
namespace HidLibrary
{
    public class HidDevice : IHidDevice
	{
		...
		protected delegate HidDeviceReport ReadReportDelegate(int timeout);
		private delegate bool WriteDelegate(byte[] data, int timeout);
		private delegate bool WriteReportDelegate(HidDeviceReport report, int timeout);
		...
		public string Description { get { return _description ?? "No description available."; } }
		...
		public HidDeviceReport ReadReport()
		{
			return ReadReport(0);
		}

		public HidDeviceReport ReadReport(int timeout)
		{
			return new HidDeviceReport(Capabilities.InputReportByteLength, Read(timeout));
		}
		...
		public async Task<HidDeviceReport> ReadReportAsync(int timeout = 0)
		{
			var readReportDelegate = new ReadReportDelegate(ReadReport);
#if NET20 || NET35 || NET5_0_OR_GREATER
			return await Task<HidDeviceReport>.Factory.StartNew(() => readReportDelegate.Invoke(timeout));
#else
            return await Task<HidDeviceReport>.Factory.FromAsync(readReportDelegate.BeginInvoke, readReportDelegate.EndInvoke, timeout, null);
#endif
		}
		...
		/// 
		public HidDeviceReport ReadReportSync(byte reportId)
		{
			...
			return new HidDeviceReport(Capabilities.InputReportByteLength, deviceData);
		}
		...
		public bool WriteReport(HidDeviceReport report)
		{
			return WriteReport(report, 0);
		}

		public bool WriteReport(HidDeviceReport report, int timeout)
		{
			return Write(report.GetBytes(), timeout);
		}

		public void WriteReport(HidDeviceReport report, WriteCallback callback)
		{
			WriteReport(report, callback, 0);
		}

		public void WriteReport(HidDeviceReport report, WriteCallback callback, int timeout)
		{
		...
		/// 
		public bool WriteReportSync(HidDeviceReport report)
		{
		...
		public async Task<bool> WriteReportAsync(HidDeviceReport report, int timeout = 0)
		{
			var writeReportDelegate = new WriteReportDelegate(WriteReport);
#if NET20 || NET35 || NET5_0_OR_GREATER
		...
		public HidDeviceReport CreateReport()
		{
			return new HidDeviceReport(Capabilities.OutputReportByteLength);
		}
